...
public class PoblacionRestController {
	...
	public ResponseEntity<?> findById(@PathVariable Long id) {
		...
		if (poblacion == null) {
			response.put("mensaje", "La población con ID: " + id.toString() + " no existe en la BBDD");
	...
	public ResponseEntity<?> update(@RequestBody Poblacion poblacion) {
		...
		Poblacion poblacionActual = poblacionService.findById(id);
		...
		if (poblacionActual == null) {
			response.put("mensaje", "Error: no se pudo editar la población con Id: "
	...
	public ResponseEntity<?> delete(@PathVariable Long id) {
		...
		try {
			...
			response.put("mensaje", "Error al elimimar la población de la BBDD");
			response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
